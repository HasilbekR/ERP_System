...
public class GroupController {
  ...
  @DeleteMapping("/{groupId}/delete")
    ...
    @PostMapping("/{attendanceId}{studentId}addStudent")
    public ResponseEntity<GroupEntity> addStudentToGroup(
            @PathVariable UUID attendanceId,
            @PathVariable UUID studentId
            ){
      return ResponseEntity.ok(groupService.addStudentToGroup(attendanceId,studentId));
    }

    @DeleteMapping("/{attendanceId}{studentId}deleteStudent")
    public ResponseEntity<GroupEntity> deleteStudentById(
            @PathVariable UUID attendanceId,
            @PathVariable UUID studentId
    ){
      return ResponseEntity.status(200).body(groupService.deleteStudentById(attendanceId,studentId));
    }
